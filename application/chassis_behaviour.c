...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    int sensor_state_left  = HAL_GPIO_ReadPin(GPIOI, GPIO_PIN_6);
    int sensor_state_right = HAL_GPIO_ReadPin(GPIOI, GPIO_PIN_7);

    if(CV_states == NOT_DETECTED)
		{
			speed = 0;
		}
		else if(CV_states == DETECTED || CV_states == FIRST_FRAM)
		{
			speed = 0;
		}
    ...
    else if(sensor_state_right && !sensor_state_left)
    {
		...
		//direction move
		if(direction == -1){
			*vy_set =  speed * CHASSIS_VY_RC_SEN;// * (arm_sin_f32(rad_in) + 1.5);
		}
		else if(direction == 1){
			*vy_set = - speed * CHASSIS_VY_RC_SEN;
		}
		else if(direction == 0){
			*vy_set = 0;
		}
